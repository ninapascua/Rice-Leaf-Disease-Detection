...
def main():
    ...
    if len(X_val_batches) == 0 or len(y_val_batches) == 0:
        raise ValueError("val_ds is empty. Check your validation dataset loading.")
    ...
    if len(train_labels) == 0:
        raise ValueError("train_ds is empty. Check your training dataset loading.")
    ...
    plot_and_save(
        episode_f1s,
        "Macro-F1 per Episode",
        "Macro-F1",
        F1_PLOT_PATH,
    )
    plot_and_save(
        episode_thresholds,
        "Selected Threshold per Episode",
        "Threshold",
        THRESHOLD_PLOT_PATH,
    )
